...
class UrgNode2 : public rclcpp_lifecycle::LifecycleNode {
   public:
     ...
     * @brief コンストラクタ
     * @details
     * クラス内メンバ変数の初期化、パラメータの宣言及び"SIGPIPE"の無効化設定を行う
     */
    explicit UrgNode2(
     ...
     * @details
     * パラメータの取得や内部の初期化およびLiDAR接続処理、スレッドの作成を実施する
     ...
     * @details
     * スレッドの停止、publisherなどの資源の解放およびLiDAR切断処理を実施する
     * @param[in] state 遷移前の状態
     * @retval CallbackReturn::SUCCESS 遷移処理成功
     */
     ...
     * @details
     * スレッドを停止させ、publisherとDiagnosticsの資源の解放およびLiDARとの接続を解除する
     * @param[in] state 遷移前の状態
     * @retval CallbackReturn::SUCCESS 遷移処理成功
     */
     ...
     * @details
     * スレッドの停止、publisherとDiagnosticsの資源の解放およびLiDAR切断処理を実施する
     * @param[in] state 遷移前の状態
     * @retval CallbackReturn::SUCCESS 遷移処理成功
     */
   ...
   private:
     ...
     * @details
     * LiDARから取得したマルチエコースキャン情報のトピックへの変換を行う
     ...
     * @brief システムレイテンシの計算
     * @details
     * 調整モード（calibrate_time_==true）時に実行、内部変数system_latency_を設定する
     ...
     * @return 補正後タイムスタンプ
     */
    rclcpp::Time get_synchronized_time(long time_stamp,
     ...
     */
    void populate_diagnostics_status(
    ...
    /** 計測モード [URG_DISTANCE or URG_DISTANCE_INTENSITY or URG_MULTIECHO or
