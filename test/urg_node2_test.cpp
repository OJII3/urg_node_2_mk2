...
void multiecho_callback(
    const sensor_msgs::msg::MultiEchoLaserScan::SharedPtr msg) {
    if (scan_flag) {
...
void scan_wait(rclcpp::executors::SingleThreadedExecutor& exe1,
               double wait_period) {
    // queue flush
    exe1.spin_some();
...
TEST(UTM_30_LX_EW, normal_scan) {
    ...
    std::vector<rclcpp::Parameter> params = {
        rclcpp::Parameter("ip_address", "192.168.0.10")};
    rclcpp::NodeOptions node_options;
    node_options.arguments(args);
    node_options.parameter_overrides(params);

    std::shared_ptr<urg_node2::UrgNode2> node =
        std::make_shared<urg_node2::UrgNode2>(node_options);
    ...
    auto subscriber =
        sub_node->create_subscription<sensor_msgs::msg::LaserScan>(
            "scan", 10, scan_callback);
    exe1.add_node(sub_node);

    // publisher test
    std::vector<rclcpp::TopicEndpointInfo> ep_scan =
    ...
    std::vector<rclcpp::TopicEndpointInfo> ep_first =
        node->get_publishers_info_by_topic("first");
    std::vector<rclcpp::TopicEndpointInfo> ep_last =
        node->get_publishers_info_by_topic("last");
    ...
    rclcpp::Time header_time =
        rclcpp::Time(scan_msg.header.stamp.sec, scan_msg.header.stamp.nanosec);
    ...
    EXPECT_NEAR(scan_msg.time_increment, 1 * (270.0 / 360.0) * 0.025 / 1080.0,
                1e-6);
...
TEST(UTM_30_LX_EW, intensity_scan) {
    ...
    std::vector<rclcpp::Parameter> params = {
        rclcpp::Parameter("ip_address", "192.168.0.10"),
        rclcpp::Parameter("angle_min", -1.57),
        rclcpp::Parameter("angle_max", 1.57),
        rclcpp::Parameter("frame_id", "hokuyo"),
        rclcpp::Parameter("publish_intensity", true),
        rclcpp::Parameter("time_offset", 10.0),
    ...
    EXPECT_NEAR(scan_msg.time_increment, 2 * (180.0 / 360.0) * 0.025 / 720.0,
                1e-6);
    EXPECT_NEAR(scan_msg.scan_time, 0.025, 1e-6);
    EXPECT_NEAR(scan_msg.range_min, 0.023, 1e-6);
    EXPECT_NEAR(scan_msg.range_max, 60.0, 1e-6);
...
TEST(UTM_30_LX_EW, multiecho_scan) {
    ...
    std::vector<rclcpp::Parameter> params = {
        ...
        rclcpp::Parameter("angle_min", 1.57),
        rclcpp::Parameter("angle_max", -1.57),
        rclcpp::Parameter("publish_multiecho", true),
        rclcpp::Parameter("skip", 2),
        rclcpp::Parameter("cluster", 0)};
    rclcpp::NodeOptions node_options;
    node_options.arguments(args);
    node_options.parameter_overrides(params);
    ...
    auto sub_node = rclcpp::Node::make_shared("test_subscription");
    auto subscriber =
        sub_node->create_subscription<sensor_msgs::msg::MultiEchoLaserScan>(
            "echoes", 10, multiecho_callback);
    exe1.add_node(sub_node);
    ...
    // compare
    rclcpp::Time header_time = rclcpp::Time(multiecho_msg.header.stamp.sec,
    ...
    EXPECT_NEAR(multiecho_msg.angle_increment, 1 * (2 * M_PI * 1) / 1440.0,
                1e-6);
    EXPECT_NEAR(multiecho_msg.time_increment,
                1 * (180.0 / 360.0) * 0.025 / 720.0, 1e-6);
    ...
    for (size_t i = 0; i < multiecho_msg.ranges.size(); i++) {
        if (multiecho_msg.ranges[i].echoes.size() < 1 ||
            3 < multiecho_msg.ranges[i].echoes.size()) {
            flag_size = true;
        }
        for (size_t j = 0; j < multiecho_msg.ranges[i].echoes.size(); j++) {
            if (multiecho_msg.ranges[i].echoes[j] ==
...
TEST(UTM_30_LX_EW, multiecho_intensity_scan) {
    ...
    std::vector<rclcpp::Parameter> params = {
        ...
        rclcpp::Parameter("publish_intensity", true),
        rclcpp::Parameter("publish_multiecho", true),
        rclcpp::Parameter("skip", 9),
        rclcpp::Parameter("cluster", 99)};
    rclcpp::NodeOptions node_options;
    node_options.arguments(args);
    node_options.parameter_overrides(params);
    ...
    auto subscriber_1 =
        sub_node->create_subscription<sensor_msgs::msg::LaserScan>(
            "first", 10, first_callback);
    auto subscriber_2 =
        sub_node->create_subscription<sensor_msgs::msg::LaserScan>(
            "last", 10, last_callback);
    auto subscriber_3 =
        sub_node->create_subscription<sensor_msgs::msg::LaserScan>(
            "most_intense", 10, most_callback);
    exe1.add_node(sub_node);
    ...
    EXPECT_NEAR(multiecho_msg.angle_increment, 99 * (2 * M_PI * 1) / 1440.0,
                1e-6);
    EXPECT_NEAR(multiecho_msg.time_increment,
                99 * (270.0 / 360.0) * 0.025 / 1080.0, 1e-6);
    EXPECT_NEAR(multiecho_msg.scan_time, 0.025, 1e-6);
    EXPECT_NEAR(multiecho_msg.range_min, 0.023, 1e-6);
    EXPECT_NEAR(multiecho_msg.range_max, 60.0, 1e-6);
    ...
    for (size_t i = 0; i < multiecho_msg.intensities.size(); i++) {
        ...
        for (size_t j = 0; j < multiecho_msg.intensities[i].echoes.size();
             j++) {
            if (multiecho_msg.intensities[i].echoes[j] ==
                std::numeric_limits<float>::quiet_NaN()) {
                flag_nan = true;
            }
        }
...
TEST(UTM_30_LX_EW, normal_scan_param_angle_min) {
    ...
    std::vector<rclcpp::Parameter> params = {
        rclcpp::Parameter("ip_address", "192.168.0.10"),
        rclcpp::Parameter("angle_min", 2.37),
...
TEST(UTM_30_LX_EW, normal_scan_param_angle_max) {
    ...
    std::vector<rclcpp::Parameter> params = {
        rclcpp::Parameter("ip_address", "192.168.0.10"),
        rclcpp::Parameter("angle_min", -2.37),
        rclcpp::Parameter("angle_max", -2.37),
        rclcpp::Parameter("cluster", -1)};
    rclcpp::NodeOptions node_options;
    node_options.arguments(args);
    node_options.parameter_overrides(params);
