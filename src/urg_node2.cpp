...
UrgNode2::UrgNode2(const rclcpp::NodeOptions& node_options)
    ...
    diagnostics_tolerance_ =
        declare_parameter<double>("diagnostics_tolerance", 0.05);
    diagnostics_window_time_ =
        declare_parameter<double>("diagnostics_window_time", 5.0);
...
void UrgNode2::initialize() {
    ...
    diagnostics_window_time_ =
        get_parameter("diagnostics_window_time").as_double();
    ...
    // 範囲チェック
    angle_min_ = (angle_min_ < -M_PI)
                     ? -M_PI
                     : ((angle_min_ > M_PI) ? M_PI : angle_min_);
    angle_max_ = (angle_max_ < -M_PI)
                     ? -M_PI
                     : ((angle_max_ > M_PI) ? M_PI : angle_max_);
    ...
    // メッセージヘッダのframe_id設定
    header_frame_id_ = (frame_id_.find_first_not_of('/') == std::string::npos)
                           ? ""
                           : frame_id_.substr(frame_id_.find_first_not_of('/'));
...
bool UrgNode2::connect() {
    if (!ip_address_.empty()) {
        // イーサネット接続
        int result =
            urg_open(&urg_, URG_ETHERNET, ip_address_.c_str(), ip_port_);
        if (result < 0) {
            RCLCPP_ERROR(get_logger(),
                         "Could not open network Hokuyo 2D LiDAR\n%s:%d\n%s",
    ...
    if (publish_intensity_) {
        ...
        if (!use_intensity_) {
            RCLCPP_WARN(
                get_logger(),
                "parameter 'publish_intensity' is true, but this device does "
                "not support intensity mode. disable intensity mode.");
        }
    }
    ...
    if (publish_multiecho_) {
        ...
        if (!use_multiecho_) {
            RCLCPP_WARN(
                get_logger(),
                "parameter 'publish_multiecho' is true, but this device does "
                "not support multiecho scan mode. switch single scan mode.");
        }
    }
...
void UrgNode2::set_scan_parameter() {
    ...
    int max_step;
    urg_step_min_max(&urg_, &min_step, &max_step);
    topic_time_increment_ =
        cluster_ *
        ((urg_step2rad(&urg_, max_step) - urg_step2rad(&urg_, min_step)) /
         (2.0 * M_PI)) *
...
bool UrgNode2::create_scan_message(sensor_msgs::msg::LaserScan& msg) {
    ...
    if (use_intensity_) {
        num_beams = urg_get_distance_intensity(&urg_, &distance_[0],
                                               &intensity_[0], &time_stamp);
    } else {
    ...
    if (synchronize_time_) {
        system_time_stamp =
            get_synchronized_time(time_stamp, system_time_stamp);
    }
...
bool UrgNode2::create_scan_message(sensor_msgs::msg::MultiEchoLaserScan& msg) {
    ...
    rclcpp::Time system_time_stamp = system_clock.now();
    if (use_intensity_) {
        num_beams = urg_get_multiecho_intensity(&urg_, &distance_[0],
                                                &intensity_[0], &time_stamp);
    } else {
        ...
        for (int j = 0; j < URG_MAX_ECHO; j++) {
            if (distance_[(URG_MAX_ECHO * i) + j] != 0) {
                distance_echo.echoes.push_back(
                    static_cast<float>(distance_[(URG_MAX_ECHO * i) + j]) /
                    1000.0f);
                if (use_intensity_) {
                    intensity_echo.echoes.push_back(
...
// 診断情報入力
void UrgNode2::populate_diagnostics_status(
...
void UrgNode2::start_diagnostics(void) {
    ...
    diagnostic_updater_->add("Hardware Status", this,
                             &UrgNode2::populate_diagnostics_status);

    // Diagnosticsトピック設定
    diagnostics_freq_ = 1.0 / (scan_period_ * (skip_ + 1));
    if (use_multiecho_) {
        echo_freq_.reset(new diagnostic_updater::HeaderlessTopicDiagnostic(
            "Laser Echoes", *diagnostic_updater_,
            diagnostic_updater::FrequencyStatusParam(
                &diagnostics_freq_, &diagnostics_freq_, diagnostics_tolerance_,
                diagnostics_window_time_)));
    } else {
        scan_freq_.reset(new diagnostic_updater::HeaderlessTopicDiagnostic(
...
void UrgNode2::stop_diagnostics(void) {
    ...
    if (has_parameter("diagnostic_updater.period")) {
        try {
            ...
            RCLCPP_WARN(get_logger(), "undeclare_parameter failed: %s",
                        e.what());
        }
    }
}
...
void UrgNode2::calibrate_system_latency(size_t num_measurements) {
    if (!is_connected_) {
        RCLCPP_WARN(get_logger(),
                    "Unable to calibrate time offset. Not Ready.");
        return;
    }
    ...
    try {
        ...
        std::nth_element(time_offsets.begin(),
                         time_offsets.begin() + time_offsets.size() / 2,
                         time_offsets.end());
        ...
        RCLCPP_INFO(get_logger(), "Calibration finished. Latency is: %.4f sec.",
                    (double)(system_latency_.nanoseconds() * 1e-9));

    } catch (const std::runtime_error& e) {
        RCLCPP_WARN(get_logger(), "Could not calibrate time offset: %s",
                    e.what());
        system_latency_ = rclcpp::Duration(0ns);
    }
}
...
rclcpp::Duration UrgNode2::get_native_clock_offset(size_t num_measurements) {
    ...
    for (size_t i = 0; i < num_measurements; i++) {
        ...
        rclcpp::Time average_time(
            (response_time.nanoseconds() + request_time.nanoseconds()) / 2.0);
...
rclcpp::Duration UrgNode2::get_time_stamp_offset(size_t num_measurements) {
    ...
    for (size_t i = 0; i < num_measurements; i++) {
        ...
        if (measurement_type_ == URG_DISTANCE) {
            ...
            ret = urg_get_distance_intensity(&urg_, &distance_[0],
                                             &intensity_[0], &time_stamp);
            ...
            ret = urg_get_multiecho_intensity(&urg_, &distance_[0],
                                              &intensity_[0], &time_stamp);
        }
...
// 指数移動平均による動的補正
rclcpp::Time UrgNode2::get_synchronized_time(long time_stamp,
                                             rclcpp::Time system_time_stamp) {
    ...
    if (adj_count_ > 0) {
        hardware_clock_adj_ =
            adj_alpha_ * cur_adj + (1.0 - adj_alpha_) * hardware_clock_adj_;
    } else {
        // 指数移動平均の初期化
    ...
    if (adj_count_ > 100) {
        ...
        if (fabs((stamp - system_time_stamp).seconds()) > 0.1) {
            ...
            RCLCPP_WARN(get_logger(), "%s: detected clock warp, reset EMA",
                        __func__);
        }
    }
    return stamp;
...
rclcpp::Duration UrgNode2::get_angular_time_offset(void) {
    ...
    if (first_step_ == 0 && last_step_ == 0) {
        ...
        circle_fraction =
            (urg_step2rad(&urg_, first_step_) + M_PI) / (2.0 * M_PI);
    }
